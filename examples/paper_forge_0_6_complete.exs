...
report_flow = fn flow ->
      ...
      |> Flow.custom(
        fn page, context ->
          ...
          entries = [
            {"01", "Performance overview", "Momentum, margin and cash conversion", "03",
             :performance, teal},
            {"02", "Operating portfolio", "Programs, regions and measured outcomes", "04",
             :portfolio, coral},
            {"03", "Rider survey & PDF craft",
             "A useful call to action, built with native vectors", "05", :capabilities, violet}
          ]
          ...
          page =
            entries
            |> Enum.with_index()
            |> Enum.reduce(page, fn {{number, title, description, page_number, destination,
                                      accent}, index},
                                    current ->
...
IO.inspect(
  ...
  label: "PaperForge 1.0 LinkedIn showcase"
)
